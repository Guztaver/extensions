...
export function useTeamDetail(teamId: string) {
	...
	const calculateExtendedData = (
		...
		const recentFormString =
			teamFormResults ||
			calculateRecentFormFromFixtures(previousMatches.slice(0, 5), teamId);

		// Get current league position from table
		const currentLeaguePosition = rawData.overview?.table?.all?.find(
			(team) => team.id.toString() === teamId,
		)?.idx;

		// Calculate countdown to next match
		const nextMatchCountdown = nextMatch
			? calculateMatchCountdown(nextMatch.status.utcTime)
			: undefined;
	...
	const calculateRecentFormFromFixtures = (
