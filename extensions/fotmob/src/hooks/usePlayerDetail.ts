	...
	const { data, error, isLoading, revalidate } = useCachedPromise(
		async (playerId: string): Promise<PlayerDetailData> => {
			...
			try {
				...
				let playerOptions =
					searchResults.squadMemberSuggest?.[0]?.options || [];

				// Try to find the player by ID in search results
				let playerResult = playerOptions.find(
					(option: { payload: { id: string } }) =>
						option.payload.id === playerId,
				);
				...
				throw new Error(
					`Unable to fetch player data: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
		[playerId],
