...
export function useSearch(searchText: string) {
  ...
  const { data, error, isLoading, revalidate } = useCachedPromise(
    async (query: string): Promise<SearchResultSection[]> => {
      ...
      // Combine all results
      const combinedResult = await Promise.all([teams, players, matches, leagues]);
      const results: SearchResultSection[] = [];

      if (combinedResult[0].length > 0) {
        results.push({
          title: "Teams",
          items: combinedResult[0] as SearchResultItem[],
        });
      }

      if (combinedResult[1].length > 0) {
        results.push({
          title: "Players",
          items: combinedResult[1] as SearchResultItem[],
        });
      }

      if (combinedResult[2].length > 0) {
        results.push({
          title: "Matches",
          items: combinedResult[2] as SearchResultItem[],
        });
      }

      if (combinedResult[3].length > 0) {
        results.push({
          title: "Leagues",
          items: combinedResult[3] as SearchResultItem[],
        });
      }
