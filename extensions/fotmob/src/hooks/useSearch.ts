...
export function useSearch(searchText: string) {
	...
	const { data, error, isLoading, revalidate } = useCachedPromise(
		async (query: string): Promise<SearchResultSection[]> => {
			...
			const players = Promise.all(
				searchResults.squadMemberSuggest?.[0]?.options.map(
					async (option): Promise<SearchResultItem> => ({
						...
						subtitle: ``,
						accessories: (
							await favoriteAccessories("player", option.payload.id)
						).concat(
							typeof option.payload.teamId === "number" &&
								option.payload.teamName
				...
				searchResults.matchSuggest?.[0]?.options.map(
					async (option): Promise<SearchResultItem> => ({
						...
						accessories: (
							...
							typeof option.payload.homeScore === "number" &&
								typeof option.payload.awayScore === "number"
								? [
										{
			...
			// Combine all results
			const cobinedResult = await Promise.all([
				teams,
				players,
				matches,
				leagues,
