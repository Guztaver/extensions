...
function TeamTableItem({
	...
	const qualColor = team.qualColor
		? getQualificationColor(team.qualColor)
		: undefined;
	const favoriteService = useFavorite();

	// Check if team is already in favorites
	const isTeamFavorited = favoriteService.teams.some(
		(favTeam) => favTeam.id === team.id.toString(),
	);
	...
	return (
		<List.Item
			...
			accessories={[
				...
				...(qualColor
					? [{ icon: { source: Icon.Circle, tintColor: qualColor } }]
					: []),
			]}
			actions={
				<ActionPanel>
					...
					<Action.OpenInBrowser
						...
						url={buildTeamDetailUrl(team.id)}
					/>
					...
					actions={
						<ActionPanel>
							<Action.OpenInBrowser
								...
								url={buildLeagueDetailUrl(leagueId)}
							/>
							<Action.CopyToClipboard
								icon={Icon.Clipboard}
