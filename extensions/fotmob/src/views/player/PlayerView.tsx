...
`}
						...
						<Action
							...
							onAction={() => {
								showToast({
									...
									title: "Tip",
									message:
										"Use 'Search Players' command to find the correct player by name",
								});
							}}
						/>
					</ActionPanel>
	...
	const generateMarkdown = () => {
		...
		// Player image
		const imageUrl =
			playerDetail.imageUrl || buildPlayerImageUrl(playerDetail.id);
		...
		if (
			playerDetail.meta?.position &&
			playerDetail.meta.position !== "Unknown"
		) {
			...
			actions={
				<ActionPanel>
					<Action.OpenInBrowser
						title="Open in Browser"
						icon={Icon.Globe}
						url={buildPlayerDetailUrl(playerDetail.id)}
					/>
					<Action
					...
					{playerDetail.primaryTeam && (
						<Action
							...
							onAction={() =>
								launchTeamCommand(
									playerDetail.primaryTeam?.id.toString() || "0",
								)
							}
						/>
					)}
