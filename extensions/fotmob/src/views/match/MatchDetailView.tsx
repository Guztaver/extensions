...
export default function MatchDetailView({ matchId }: MatchDetailViewProps) {
	...
	const matchSummary = useMemo(() => {
		if (!data?.status || !data?.home || !data?.away || !data?.tournament)
			return null;
	...
	const matchEvents = useMemo(() => {
		...
		return data.events
			...
			.map((event) => ({
				...event,
				displayMinute: event.minuteExtra
					...
					actions={
						<ActionPanel>
							<Action.OpenInBrowser
								icon={Icon.Globe}
								title="View on Fotmob"
								url={buildMatchDetailUrl(matchId)}
							/>
						</ActionPanel>
					}
				/>
		...
		>
			...
			<List.Section title="Match Info">
				...
				<List.Item
					...
					accessories={
						data.tournament.round ? [{ text: data.tournament.round }] : []
					}
					actions={
						<ActionPanel>
							<Action.OpenInBrowser
							...
							<Action.CopyToClipboard
								...
								shortcut={{ modifiers: ["cmd"], key: "." }}
							/>
							{favoriteService.leagues.some(
								(league) => league.id === data.tournament.leagueId.toString(),
							) ? (
								<Action
									icon={Icon.StarDisabled}
									title="Remove League From Favorites"
									onAction={async () => {
										await favoriteService.removeItems(
											"league",
											data.tournament.leagueId.toString(),
										);
										showToast({
				...
				{data?.referee && (
					<List.Item
						...
						accessories={
							data.referee.country ? [{ text: data.referee.country }] : []
						}
					/>
				)}
				...
				{data?.attendance && (
					<List.Item
						icon={Icon.TwoPeople}
						title="Attendance"
						subtitle={data.attendance.toLocaleString()}
					/>
				)}
			</List.Section>
						...
						actions={
							<ActionPanel>
								...
								<Action.CopyToClipboard
									...
									shortcut={{ modifiers: ["cmd"], key: "." }}
								/>
								{favoriteService.teams.some(
									(team) => team.id === data.home.id.toString(),
								) ? (
									<Action
										icon={Icon.StarDisabled}
					...
					<List.Item
						...
						actions={
							<ActionPanel>
								...
								/>
								{favoriteService.teams.some(
									(team) => team.id === data.away.id.toString(),
								) ? (
									<Action
										icon={Icon.StarDisabled}
										title="Remove From Favorites"
				...
				<List.Section title="Additional Info">
					{data.tournament.season && (
						<List.Item
							icon={Icon.Calendar}
							title="Season"
							subtitle={data.tournament.season}
						/>
					)}
