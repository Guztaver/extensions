...
export default function MatchdayView() {
	...
	if (
		query.trim().length > 0 &&
		((searchState.result ?? []).length > 0 || !searchState.isLoading)
	) {
		return (
			<List
											...
											{item.type === "team" ? (
												...
												<>
													...
													{favoriteService.leagues.some(
														(league) => league.id === item.payload.id,
													) ? (
														<Action
															icon={Icon.StarDisabled}
															title="Remove From Favorites"
												...
												<>
													...
													<Action.CopyToClipboard
														...
														shortcut={{ modifiers: ["cmd"], key: "." }}
													/>
													{favoriteService.players.some(
														(player) => player.id === item.payload.id,
													) ? (
														<Action
															icon={Icon.StarDisabled}
															title="Remove From Favorites"
