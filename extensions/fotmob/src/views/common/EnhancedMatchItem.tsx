...
export type EnhancedMatchItemProps = Omit<
...
export default function EnhancedMatchItem({
	...
	if (!match.home.name || !match.away.name) {
		return (
			<SafeListItem
				title="Unknown Teams"
				subtitle="Team names are missing"
				icon={Icon.ExclamationMark}
				{...rest}
			/>
		);
	...
	const buildDefaultActions = () => {
		const isHomeTeamFavorited = favoriteService.teams.some(
			(team) => team.id === match.home?.id?.toString(),
		);
		const isAwayTeamFavorited = favoriteService.teams.some(
			(team) => team.id === match.away?.id?.toString(),
		);
			...
			<ActionPanel>
				<ActionPanel.Section title="Match Actions">
					...
					<Action.OpenInBrowser
						icon={Icon.Globe}
						title="View Match on Fotmob"
						url={buildMatchDetailUrl(match.id)}
					/>
				...
				{additionalActions && (
					<ActionPanel.Section title="Additional Actions">
						{additionalActions}
					</ActionPanel.Section>
				)}
			</ActionPanel>
