...
export default function TeamView({ id }: { id: string }) {
	...
	const safeTeamName =
		teamName?.trim() && teamName !== "undefined" && teamName !== "null"
			? teamName
			: `Team ${id}`;
		...
		<List navigationTitle={`${safeTeamName} - Team Details`}>
			...
			<List.Section title="Team Overview">
				<SafeListItem
					...
					accessories={[
						...(data?.details?.shortName
							? [{ text: data.details.shortName }]
							: []),
				...
				{data?.overview?.venue?.name && (
					<SafeListItem
						...
						accessories={[
							...
							...(data.overview.venue.opened
								? [{ text: `Opened: ${data.overview.venue.opened}` }]
								: []),
						]}
					/>
				)}
				...
				{data?.details?.primaryLeagueName && (
					<SafeListItem
						...
						icon={
							data.details.primaryLeagueId
								? buildLeagueLogoUrl(data.details.primaryLeagueId, "dark")
								: Icon.Trophy
						}
				...
				{/* Team Form */}
				{(data?.calculated?.teamFormResults ||
					data?.calculated?.recentFormString) && (
					<SafeListItem
						title="Recent Form"
						subtitle={
							formatFormString(
				...
				{data?.calculated?.upcomingMatch &&
					data?.calculated?.nextMatchCountdown && (
						<SafeListItem
							title="Next Match"
							...
							accessories={[
								...
								{
									text: prettyDate(
										data.calculated.upcomingMatch.status?.utcTime || "",
									),
								},
							]}
						/>
			...
			{data?.overview?.table?.all &&
				Array.isArray(data.overview.table.all) &&
				data.overview.table.all.length > 0 && (
			...
			{/* Top Players */}
			{data?.overview?.topPlayers &&
				Array.isArray(data.overview.topPlayers) &&
				data.overview.topPlayers.length > 0 && (
			...
			{(() => {
				...
				if (!hasValidPrimary && !hasValidSecondary && !hasValidText)
					return null;
				...
				return (
					<List.Section title="Team Colors">
						<SafeListItem
							...
							accessories={[
								...(hasValidText ? [{ text: `Text: ${colors.text}` }] : []),
							]}
						/>
					</List.Section>
				);
			...
			{nextMatches?.length > 0 &&
				nextMatches.filter((match) => match?.id).length > 0 && (
			...
			{lastMatches?.length > 0 &&
				lastMatches.filter((match) => match?.id).length > 0 && (
