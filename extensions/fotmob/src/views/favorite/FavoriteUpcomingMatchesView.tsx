...
function useTeamUpcomingMatches(teamId: string) {
	...
	const upcomingMatches = useMemo(() => {
		...
		if (
			data.calculated.upcommingMatch &&
			!data.calculated.upcommingMatch.status.finished
		) {
...
export default function FavoriteUpcomingMatchesView() {
							...
							<Action
								...
								onAction={() => {
									...
									showToast({
										...
										title:
											"Use the search command to find and add teams to favorites",
									});
								}}
							/>
						</ActionPanel>
	...
	return (
		<List
			...
			isLoading={false}
		>
			{favoriteService.teams.map(
				(team: { id: string; name: string; leagueId: string }) => (
